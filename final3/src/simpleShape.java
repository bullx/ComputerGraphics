...
public class simpleShape {
	...
	/**
	 * add a triangle to the current shape using calculated normals
	 */
	protected void addTriangle(float x0, float y0, float z0, float u0,
		...
		// Attach the normal to all 3 vertices
		addTriangleWithNorms(x0, y0, z0, x1, y1, z1, x2, y2, z2, 3 * nx,
				3 * ny, 3 * nz, 3 * nx, 3 * ny, 3 * nz, 3 * nx, 3 * ny, 3 * nz);
		uv.add(new Float(u0));
		uv.add(new Float(v0));
		uv.add(new Float(u1));
		uv.add(new Float(v1));
	...
	protected void addTriangle1(float x0, float y0, float z0, float x1,
		...
		float ny = (uz * vx) - (ux * vz);
		float nz = (ux * vy) - (uy * vx);
		addTriangleWithNorms(x0, y0, z0, x1, y1, z1, x2, y2, z2, nx, ny, nz,
				nx, ny, nz, nx, ny, nz);
	}
	 ...
	 */
	protected void addTriangleWithNorms(float x0, float y0, float z0, float x1,
			float y1, float z1, float x2, float y2, float z2, float nx0,
			float ny0, float nz0, float nx1, float ny1, float nz1, float nx2,
			float ny2, float nz2) {
		points.add(new Float(x0));
		points.add(new Float(y0));
		points.add(new Float(z0));
